...
describe("loadAudioBuffer", () => {
    ...
    const mockAudioBuffer = { duration: 1.0 }; // Simplified mock AudioBuffer
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    it("should load and decode an audio buffer successfully", async () => {
        ...
        const result = await loadAudioBuffer(mockAudioContext, mockUrl);
    ...
    it("should throw an error if fetch fails", async () => {
        ...
        await expect(loadAudioBuffer(mockAudioContext, mockUrl)).rejects.toThrow("HTTP error! status: 404 for URL: http://example.com/audio.wav");
    ...
    it("should throw an error if decodeAudioData fails", async () => {
        ...
        await expect(loadAudioBuffer(mockAudioContext, mockUrl)).rejects.toThrow("Decoding failed");
        expect(mockFetch).toHaveBeenCalledWith(mockUrl);
        expect(mockAudioContext.decodeAudioData).toHaveBeenCalledWith(mockArrayBuffer);
        expect(console.error).toHaveBeenCalled();
    });
